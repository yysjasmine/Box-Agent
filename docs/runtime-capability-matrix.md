...
| `llm/model_profiles.py` / `llm/binding.py` | Immutable model-profile revision validation and per-run Provider binding without persistent credentials |
...
| `tools/skillhub_*` | Host-negotiated, session-composed SkillHub search and confirmed installation; no Kernel branch |
| `tools/pptx_safety.py` | Tool-boundary PPTX self-check and image-status bypass rejection |
...
| `workflows/controlled_presentation.py` | Native controlled-PPT policy: applies per-run research/image options, checkpoints pending chunk writes and stage/evidence state, rejects repair stalls, contributes the registered `pptx` Skill context, enforces the delivery budget, and publishes pause/terminal lifecycle facts |
...
| Host SkillHub search/install | `SkillHubToolContributor` + ACP host extension bridge | Available only after the host advertises matching capability versions; search is bounded per Run and install requires candidate binding plus user confirmation |
| Immutable model profiles and hosted credential refresh | `llm/model_profiles.py`, `LLMClientPort`, `auth.py` | Available; the selected profile revision is persisted as non-secret identity while hosted JWTs refresh once across concurrent calls and remain outside events |
