...
| MCP/SkillHub loading, managed bootstrap, `mcp_config`, `tool_search`, catalog or hot reload | [Development and extension guide](../DEVELOPMENT_GUIDE.md), [production guide](../PRODUCTION_GUIDE.md), and the checked-in `box_agent/config/config-example.yaml` | CLI and ACP share the user-owned MCP config. Deferred MCP aliases preserve remote names and fail closed on collisions. SkillHub tools are session-composed only after host capability negotiation; search is bounded and install requires a bound candidate plus confirmation. | `tests/test_mcp_bootstrap.py`, `tests/test_mcp.py`, `tests/test_mcp_tool_search.py`, `tests/test_skillhub_*`, runtime packaging tests, CLI and ACP wiring tests |
...
| Controlled PPTX routing, research, checkpoints, repair, rendering, or document Skills | [Controlled HTML PPTX architecture](../PPTX_CONTROLLED_HTML_ARCHITECTURE.md) / [受控 HTML PPTX 架构](../PPTX_CONTROLLED_HTML_ARCHITECTURE_CN.md), then [development guide](../PPTX_CONTROLLED_HTML_DEVELOPMENT.md) / [开发指南](../PPTX_CONTROLLED_HTML_DEVELOPMENT_CN.md) | PPT-specific recognition, pending-write checkpoint state, recovery, repair-stall detection, tool restrictions, and evidence stay in workflows/Skills rather than the generic kernel. Shell/Jupyter guards also block self-check and image-status bypass without relying on prompt compliance. | PPT contract tests, manifest checks when a built-in Skill changes, and render/visual/runtime proof when behavior is user-visible |
